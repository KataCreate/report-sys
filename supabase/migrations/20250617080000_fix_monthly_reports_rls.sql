...
CREATE POLICY "Allow all access for authenticated users" ON monthly_reports
...
CREATE POLICY "Allow all access for authenticated users" ON video_analytics
  FOR ALL USING (auth.role() = 'authenticated');
...
CREATE POLICY "Allow all access for authenticated users" ON channels
  FOR ALL USING (auth.role() = 'authenticated');